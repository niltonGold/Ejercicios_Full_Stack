...
export async function getTVCtrl(req, res) {//
    ...
    // const tvs = await retrieveTVs();// retrieveTVS me devuelve todoss
    // puedo llamar a mi funcion de filtros porque igual si no le aplico filtros me devuelve todas las tvs
    
    const {brand, inches} = req.query;// deconstruccion del objeto query solo quedandome con los query

    const tvs = await retrieveTVByBrandAndInches(brand, parseInt(inches));// debo pasarle el brand que viene del query param
